...
| zeroDateTimeBehavior | String | exception | JAVA连接MySQL数据库,在操作值为0的timestamp类型时不能正确的处理,而是默认抛出一个异常。<br>参数:<br>exception：默认值<br>convertToNull：将日期转换成NULL值<br>round：替换成最近的日期                                                  |
