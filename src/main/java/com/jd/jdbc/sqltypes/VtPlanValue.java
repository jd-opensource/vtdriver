...
public class VtPlanValue {
     ...
     * @throws SQLException
     */
    public VtValue resolveValue(Map<String, BindVariable> bindVariableMap) throws SQLException {
        if (!StringUtil.isNullOrEmpty(this.key)) {
            BindVariable bv = this.lookupValue(bindVariableMap);
            return VtValue.newVtValue(bv.getType(), bv.getValue());
     ...
     * @throws SQLException
     */
    public List<VtValue> resolveList(Map<String, BindVariable> bindVariableMap) throws SQLException {
        if (!StringUtil.isNullOrEmpty(this.listKey)) {
            BindVariable bv = this.lookupList(bindVariableMap);
     ...
     * @throws SQLException
     */
    private BindVariable lookupValue(Map<String, BindVariable> bindVariableMap) throws SQLException {
        if (!bindVariableMap.containsKey(this.key)) {
            throw new SQLException("missing bind var " + this.key);
        }
        BindVariable bv = bindVariableMap.get(this.key);
     ...
     * @throws SQLException
     */
    private BindVariable lookupList(Map<String, BindVariable> bindVariableMap) throws SQLException {
        if (!bindVariableMap.containsKey(this.listKey)) {
            throw new SQLException("missing bind var " + this.listKey);
        }
        BindVariable bv = bindVariableMap.get(this.listKey);
        if (!Query.Type.TUPLE.equals(bv.getType())) {
