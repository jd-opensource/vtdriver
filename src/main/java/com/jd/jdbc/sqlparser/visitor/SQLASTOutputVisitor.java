...
import com.jd.jdbc.sqlparser.ast.statement.SQLCreateTriggerStatement.TriggerType;
import com.jd.jdbc.sqlparser.ast.statement.SQLInsertStatement.ValuesClause;
import com.jd.jdbc.sqlparser.ast.statement.SQLJoinTableSource.JoinType;
import com.jd.jdbc.sqlparser.ast.statement.SQLMergeStatement.MergeInsertClause;
...
import static com.jd.jdbc.sqlparser.utils.Utils.getBoolean;

public class SQLASTOutputVisitor extends SQLASTVisitorAdapter implements ParameterizedVisitor, PrintableVisitor {
    private final Keywords keywords = Keywords.DEFAULT_KEYWORDS;
    public static Boolean defaultPrintStatementAfterSemi;

    static {
        try {
            defaultPrintStatementAfterSemi = getBoolean(System.getProperties(), "sql.output.printStatementAfterSemi"); // compatible for early versions
        } catch (Throwable ex) {
            // skip
        }
    }
        ...
        for (int i = 0, lineItemCount = 0, size = selectList.size()
            ...
            if (i != 0) {
                ...
                if (preSelectItem.getAfterCommentsDirect() != null) {
                    ...
                    if (lineItemCount >= selectListNumberOfLine && isEnabled(VisitorFeature.OutputPrettyFormat)) {
                        lineItemCount = paramCount;
                        println();
                    }
                } else if ((lineItemCount >= selectListNumberOfLine
                        || selectItemExpr instanceof SQLQueryExpr
                        || selectItemExpr instanceof SQLCaseExpr) && isEnabled(VisitorFeature.OutputPrettyFormat)) {
                    lineItemCount = 0;
                    println();
                }
    ...
    protected final void printExpr(SQLExpr x) {
        ...
        if (clazz == SQLIdentifierExpr.class) {
            ...
            visit((SQLNotExpr) x);
        } else if (clazz == VtArgument.class) {
            visit((VtArgument) x);
        } else {
            x.accept(this);
        }
    ...
    public boolean visit(SQLIdentifierExpr x) {
        ...
        if (keywords.getKeyword(normalizedName.toUpperCase()) == null) {
            print0(normalizedName);
        } else {
            print0(originalName);
        }
        return false;
    ...
    public boolean visit(SQLLimit x) {
        ...
        if (x.isLimitOffset()) {
            printExpr(x.getRowCount());
            print0(" ");
            print0(ucase ? "OFFSET " : "offset ");
            printExpr(x.getOffset());
        } else {
    ...
    public boolean visit(VtArgument x) {
        print0(new String(x.getValue()));
        return false;
    }
}
