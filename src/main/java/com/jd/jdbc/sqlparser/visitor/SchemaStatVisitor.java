...
import com.jd.jdbc.sqlparser.dialect.mysql.ast.expr.MySqlExpr;
import com.jd.jdbc.sqlparser.dialect.mysql.visitor.MySqlASTVisitorAdapter;
import com.jd.jdbc.sqlparser.repository.SchemaObject;
import com.jd.jdbc.sqlparser.repository.SchemaRepository;
import com.jd.jdbc.sqlparser.stat.TableStat;
import com.jd.jdbc.sqlparser.stat.TableStat.Column;
import com.jd.jdbc.sqlparser.stat.TableStat.Condition;
import com.jd.jdbc.sqlparser.stat.TableStat.Mode;
import com.jd.jdbc.sqlparser.stat.TableStat.Relationship;
...
public class SchemaStatVisitor extends SQLASTVisitorAdapter {
    ...
    protected final Map<Long, SQLName> sqlNames = new LinkedHashMap<>();
    ...
    protected Column addColumn(String tableName, String columnName, SQLName expr) {
        Column column = this.getColumn(tableName, columnName);
        if (column == null && columnName != null) {
            column = new Column(tableName, columnName);
            columns.put(column.hashCode64(), column);
            sqlNames.put(column.hashCode64(), expr);
        }
        return column;
    }

    protected Column addColumn(SQLName table, String columnName, SQLName expr) {
        String tableName = table.toString();
        long tableHashCode64 = table.hashCode64();
        ...
        if (column == null && columnName != null) {
            column = new Column(tableName, columnName, columnHashCode64);
            columns.put(columnHashCode64, column);
            sqlNames.put(columnHashCode64, expr);
        }
        return column;
    }
...
//        }
    ...
    public boolean visit(SQLPropertyExpr x) {
        ...
        if (tableSource instanceof SQLExprTableSource) {
            ...
            if (expr instanceof SQLIdentifierExpr) {
                ...
                column = addColumn(table.getName(), ident, (SQLIdentifierExpr) expr);
                ...
                SQLPropertyExpr table = (SQLPropertyExpr) expr;
                String tableName = table.toString();
                column = addColumn(tableName, ident, (SQLPropertyExpr) expr);
                ...
                if ("table".equalsIgnoreCase(methodInvokeExpr.getMethodName())
                    ...
                    column = addColumn(tableName, ident, table);
                }
            }
        } else if (tableSource instanceof SQLWithSubqueryClause.Entry
            ...
            for (SQLObject parent = x.getParent(); parent != null; parent = parent.getParent()) {
                if (parent instanceof SQLSelectQueryBlock) {
                    SQLTableSource from = ((SQLSelectQueryBlock) parent).getFrom();
                    if (StringUtil.isNullOrEmpty(from.getAlias())) {
                        if (((SQLExprTableSource) from).getName().getSimpleName().equals(x.getOwnernName())) {
                            column = addColumn(x.getOwnernName(), ident, x);
                            skip = true;
                        }
                    } else if (from.getAlias().equals(x.getOwnernName())) {
                        column = addColumn(x.getOwnernName(), ident, x);
                        skip = true;
                    }
                } else if (parent instanceof SQLSelectQuery) {
            ...
            if (!skip) {
                column = handleUnkownColumn(ident, x);
            }
        }
    ...
    public boolean visit(SQLIdentifierExpr x) {
        ...
        if (tableSource instanceof SQLExprTableSource) {
            ...
            if (expr instanceof SQLIdentifierExpr) {
                SQLIdentifierExpr table = (SQLIdentifierExpr) expr;
                column = addColumn(table, ident, (SQLIdentifierExpr) expr);
    ...
    protected Column handleUnkownColumn(String columnName, SQLName expr) {
        return addColumn("UNKNOWN", columnName, expr);
    }
    ...
    public Collection<SQLName> getSqlNames() {
        return sqlNames.values();
    }
