...
public class VtTableRouteWireupVarFormatVisitor extends MySqlASTVisitorAdapter {
    private final TableRoutePlan trb;

    private final Jointab jt;

    private final Builder bldr;

    public VtTableRouteWireupVarFormatVisitor(final TableRoutePlan trb, final Jointab jt, final Builder bldr) {
        this.trb = trb;
        this.jt = jt;
        this.bldr = bldr;
    }
    ...
    private void format(final SQLName x) {
        try {
            if (!this.trb.isLocal(x)) {
                String joinVar = this.jt.procure(this.bldr, x, this.trb.order());
