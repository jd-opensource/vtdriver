...
public class VtTableRouteWireupFixUpAstVisitor extends MySqlASTVisitorAdapter {
    private final TableRoutePlan trb;

    public VtTableRouteWireupFixUpAstVisitor(final TableRoutePlan trb) {
        this.trb = trb;
    }
    ...
    public boolean visit(final SQLBinaryOpExpr x) {
        ...
        if (SQLBinaryOperator.Equality.equals(x.getOperator())) {
            if (this.trb.exprIsValue(leftValue) && !this.trb.exprIsValue(rightValue)) {
