...
package com.jd.jdbc.table.streamquery;
...
public class StreamQueryTest extends TestSuite {
    @Rule
    public final ExpectedException expection = ExpectedException.none();

    protected Connection conn;

    protected List<StreamQueryTest.TestCase> testCaseList;

    @Before
    public void init() throws SQLException, IOException {
        conn = getConnection(TestSuite.Driver.of(TestSuiteShardSpec.TWO_SHARDS));
        testCaseList = new ArrayList<>();
        testCaseList.addAll(iterateExecFile("src/test/resources/engine/tableengine/streamquery/select_case.json", StreamQueryTest.TestCase.class));
    }
    ...
    protected void printResultRow(StreamQueryTest.ResultRow[] rows, String message) {
        printNormal(message);
        for (StreamQueryTest.ResultRow row : rows) {
    ...
    public void test_select()
        ...
        expection.expect(SQLFeatureNotSupportedException.class);
        expection.expectMessage("unsupported streamExecute");
        int count = 0;
        for (StreamQueryTest.TestCase testCase : testCaseList) {
            count++;
            ...
            try (Statement stmt = conn.createStatement()) {
                ...
                List<StreamQueryTest.ResultRow> queryResult = new ArrayList<>();
                try {
                    stmt.setFetchSize(Integer.MIN_VALUE);
                    ResultSet rs = stmt.executeQuery(testCase.getQuery());
                    while (rs.next()) {
                        StreamQueryTest.ResultRow row = new StreamQueryTest.ResultRow();
                ...
                StreamQueryTest.ResultRow[] expected = testCase.getVerifyResult();
                StreamQueryTest.ResultRow[] actual = queryResult.toArray(new StreamQueryTest.ResultRow[0]);
    ...
    protected static class TestCase extends TestSuiteCase {
        ...
        StreamQueryTest.ResultRow[] verifyResult;

        String exception;
    }

    @Setter
    protected static class ResultRow implements Comparable<StreamQueryTest.ResultRow> {
        ...
        @Override
        public int compareTo(StreamQueryTest.ResultRow o) {
        ...
        public boolean equals(Object other) {
            if (!(other instanceof StreamQueryTest.ResultRow)) {
                return false;
            }

            StreamQueryTest.ResultRow o = (StreamQueryTest.ResultRow) other;
